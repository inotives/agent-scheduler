...
def test_builds_prefect_cron_schedule() -> None:
    schedule = build_prefect_schedule(
        CronScheduleSpec(cron="0 16 * * *", timezone="Asia/Singapore")
    )
...
def test_invalid_cron_is_rejected() -> None:
    with pytest.raises(ValueError):
        build_prefect_schedule(CronScheduleSpec(cron="bad", timezone="Asia/Singapore"))
...
def test_builds_one_shot_schedule_as_single_rrule() -> None:
    ...
    assert "DTSTART:20260527T000000" in schedule.rrule
    assert schedule.timezone == "Asia/Singapore"
